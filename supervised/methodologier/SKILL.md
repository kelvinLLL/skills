...
```json
{"timestamp":"<ISO-8601 UTC>","skill": "methodologier","workspace":"<workspace basename or unknown>","trigger":"<short non-sensitive reason>","task_type":"<planning|implementation|review|research|other>","outcome":"<started|completed|blocked|skipped>","verification":"<passed|failed|not_run|not_applicable>"}
```
...
```bash
python3 ~/.codex/skills/skill-supervisor/scripts/log_usage.py --skill "methodologier" --trigger "<short non-sensitive reason>" --task-type "<type>" --outcome "<outcome>" --verification "<status>"
```
